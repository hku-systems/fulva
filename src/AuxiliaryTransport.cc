 ...
 */
AuxiliaryTransport::AuxiliaryTransport(Context *context,
                                       const ServiceLocator *locator,
                                       Driver *driver, bool driverOwner,
                                       uint64_t clientId)
    : context(context), driver(driver), driverOwner(driverOwner),
    ...
    // is observed.
    , smallMessageThreshold(300), clientId(clientId),
      nextClientSequenceNumber(1), nextServerSequenceNumber(1),
      receivedPackets(), messagesToGrant(), serverRpcPool(), clientRpcPool(),
      outgoingRpcs(), outgoingRequests(), topOutgoingMessages(),
      transmitDataSlowPath(true), incomingRpcs(), outgoingResponses(),
      serverTimerList(), roundTripBytes(getRoundTripBytes(locator)),
      grantIncrement(5 * maxDataPerPacket), timerInterval(0),
      nextTimeoutCheck(0), timeoutCheckDeadline(0)
    ...
    , timeoutIntervals(40), pingIntervals(3)
{
...
string
AuxiliaryTransport::getServiceLocator()
{
 ...
 */
void
AuxiliaryTransport::deleteClientRpc(ClientRpc *clientRpc)
{
 ...
 */
void
AuxiliaryTransport::deleteServerRpc(ServerRpc *serverRpc)
{
 ...
 */
uint32_t
AuxiliaryTransport::getRoundTripBytes(const ServiceLocator *locator)
{
    ...
    roundTripBytes =
        ((roundTripBytes + maxDataPerPacket - 1) / maxDataPerPacket)
        * maxDataPerPacket;
 ...
 */
string
AuxiliaryTransport::opcodeSymbol(uint8_t opcode)
{
    switch (opcode) {
        case AuxiliaryTransport::PacketOpcode::ALL_DATA:
            return "ALL_DATA";
        case AuxiliaryTransport::PacketOpcode::DATA:
            return "DATA";
        case AuxiliaryTransport::PacketOpcode::GRANT:
            return "GRANT";
        case AuxiliaryTransport::PacketOpcode::LOG_TIME_TRACE:
            return "LOG_TIME_TRACE";
        case AuxiliaryTransport::PacketOpcode::RESEND:
            return "RESEND";
        case AuxiliaryTransport::PacketOpcode::BUSY:
            return "BUSY";
        case AuxiliaryTransport::PacketOpcode::ABORT:
            return "ABORT";
    }
 ...
 */
uint32_t
AuxiliaryTransport::sendBytes(const Driver::Address *address, RpcId rpcId,
                              Buffer *message, uint32_t offset,
                              uint32_t maxBytes,
...
template<typename T>
void
AuxiliaryTransport::sendControlPacket(const Driver::Address *recipient,
                                      const T *packet)
{
    QueueEstimator::TransmitQueueState txQueueState;
 ...
 */
string
AuxiliaryTransport::headerToString(const void *packet, uint32_t packetLength)
{
    string result;
    const AuxiliaryTransport::CommonHeader *common =
        static_cast<const AuxiliaryTransport::CommonHeader *>(packet);
    uint32_t headerLength = sizeof32(AuxiliaryTransport::CommonHeader);
    if (packetLength < headerLength) {
        goto packetTooShort;
    }
    result += AuxiliaryTransport::opcodeSymbol(common->opcode);
    if (common->flags & AuxiliaryTransport::FROM_CLIENT) {
        result += " FROM_CLIENT";
    } else {
        result += " FROM_SERVER";
    }
    ...
    switch (common->opcode) {
        case AuxiliaryTransport::PacketOpcode::ALL_DATA:
            headerLength = sizeof32(AuxiliaryTransport::AllDataHeader);
            if (packetLength < headerLength) {
                goto packetTooShort;
            }
            break;
        case AuxiliaryTransport::PacketOpcode::DATA: {
            headerLength = sizeof32(AuxiliaryTransport::DataHeader);
            if (packetLength < headerLength) {
                goto packetTooShort;
            }
            const AuxiliaryTransport::DataHeader *data =
                static_cast<const AuxiliaryTransport::DataHeader *>(packet);
            result += format(", totalLength %u, offset %u%s",
                             data->totalLength, data->offset,
                             common->flags & AuxiliaryTransport::RETRANSMISSION
                             ? ", RETRANSMISSION" : "");
            break;
        }
        case AuxiliaryTransport::PacketOpcode::GRANT: {
            headerLength = sizeof32(AuxiliaryTransport::GrantHeader);
            if (packetLength < headerLength) {
                goto packetTooShort;
            }
            const AuxiliaryTransport::GrantHeader *grant =
                static_cast<const AuxiliaryTransport::GrantHeader *>(packet);
            result += format(", offset %u", grant->offset);
            break;
        }
        case AuxiliaryTransport::PacketOpcode::LOG_TIME_TRACE:
            headerLength = sizeof32(AuxiliaryTransport::LogTimeTraceHeader);
            if (packetLength < headerLength) {
                goto packetTooShort;
            }
            break;
        case AuxiliaryTransport::PacketOpcode::RESEND: {
            headerLength = sizeof32(AuxiliaryTransport::ResendHeader);
            if (packetLength < headerLength) {
                goto packetTooShort;
            }
            const AuxiliaryTransport::ResendHeader *resend =
                static_cast<const AuxiliaryTransport::ResendHeader *>(
                    packet);
            result += format(", offset %u, length %u%s",
                             resend->offset, resend->length,
                             common->flags & AuxiliaryTransport::RESTART
                             ? ", RESTART" : "");
            break;
        }
        case AuxiliaryTransport::PacketOpcode::BUSY: {
            headerLength = sizeof32(AuxiliaryTransport::BusyHeader);
            if (packetLength < headerLength) {
                goto packetTooShort;
            }
            break;
        ...
        case AuxiliaryTransport::PacketOpcode::ABORT: {
            headerLength = sizeof32(AuxiliaryTransport::AbortHeader);
            if (packetLength < headerLength) {
                goto packetTooShort;
            }
            break;
 ...
 */
uint32_t
AuxiliaryTransport::tryToTransmitData()
{
    ...
    int transmitQueueSpace =
        driver->getTransmitQueueSpace(context->auxDispatch->currentTime);
 ...
 */
void
AuxiliaryTransport::maintainTopOutgoingMessages(OutgoingMessage *candidate)
{
 ...
 */
AuxiliaryTransport::Session::Session(AuxiliaryTransport *t,
                                     const ServiceLocator *locator,
                                     uint32_t timeoutMs)
    : Transport::Session(locator->getOriginalString()), t(t),
      serverAddress(NULL), aborted(false)
{
    try {
 ...
 */
AuxiliaryTransport::Session::~Session()
{
    abort();
...
void
AuxiliaryTransport::Session::abort()
{
    aborted = true;
...
void
AuxiliaryTransport::Session::cancelRequest(RpcNotifier *notifier)
{
    for (ClientRpcMap::iterator it = t->outgoingRpcs.begin();
...
string
AuxiliaryTransport::Session::getRpcInfo()
{
    string result;
    for (ClientRpcMap::iterator it = t->outgoingRpcs.begin();
         it != t->outgoingRpcs.end(); it++) {
...
void
AuxiliaryTransport::Session::sendRequest(Buffer *request, Buffer *response,
 ...
 */
void
AuxiliaryTransport::handlePacket(Driver::Received *received)
{
    ...
    if (!(common->flags & FROM_CLIENT)) {
        ...
        switch (common->opcode) {
            ...
            case PacketOpcode::ALL_DATA: {
                ...
                context->workerManager->handoffRpc(serverRpc);
                return;
            }
            ...
            case PacketOpcode::GRANT: {
                ...
                if ((serverRpc == NULL) || !serverRpc->sendingResponse) {
                    RAMCLOUD_LOG(WARNING, "unexpected GRANT from client %s, "
                                 ...
                                 header->common.rpcId.clientId,
                                 header->common.rpcId.sequence, header->offset,
                                 serverRpc ? "not sending response"
                                           : "not found");
                    return;
                }
 ...
 */
string
AuxiliaryTransport::ServerRpc::getClientServiceLocator()
{
 ...
 */
void
AuxiliaryTransport::ServerRpc::sendReply()
{
 ...
 */
AuxiliaryTransport::MessageAccumulator::MessageAccumulator(
    AuxiliaryTransport *t,
    Buffer *buffer)
    : t(t), buffer(buffer), fragments()
{
 ...
 */
bool
AuxiliaryTransport::MessageAccumulator::addPacket(DataHeader *header,
                                                  uint32_t length)
{
 ...
 */
uint32_t
AuxiliaryTransport::MessageAccumulator::requestRetransmission(
    AuxiliaryTransport *t,
    ...
    uint8_t whoFrom)
{
    ...
    if (endOffset <= buffer->size()) {
            LOG(ERROR, "Bad endOffset %u, offset %u", endOffset,
                buffer->size());
    }
 ...
 */
AuxiliaryTransport::ScheduledMessage::ScheduledMessage(RpcId rpcId,
                                                       MessageAccumulator *accumulator,
                                                       uint32_t unscheduledBytes,
                                                       const Driver::Address *senderAddress,
                                                       uint32_t totalLength,
                                                       uint8_t whoFrom)
    : accumulator(accumulator), grantOffset(unscheduledBytes), rpcId(rpcId),
      senderAddress(senderAddress), totalLength(totalLength), whoFrom(whoFrom)
{}

/**
 * Destructor for ScheduledMessages.
 */
AuxiliaryTransport::ScheduledMessage::~ScheduledMessage()
{}
 ...
 */
int
AuxiliaryTransport::Poller::poll()
{
    int result = 0;
 ...
 */
void
AuxiliaryTransport::checkTimeouts()
{
