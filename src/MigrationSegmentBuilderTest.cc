...
struct MigrationSegmentBuilderTest : public ::testing::Test {
    ...
    MigrationSegmentBuilderTest()
        : context(), serverId(99, 0), serverList(&context),
...
TEST_F(MigrationSegmentBuilderTest, build)
{
    ...
    { // RpcResult should go in partition 1.
        Key key(2, "1", 1);
        Buffer dataBuffer;
        RpcResult rpcResult(2, key.getHash(), 6, 4, 2, dataBuffer);
        Buffer buffer;
    ...
    { // TxDecisionRecord should go in partition 1.
        Key key(2, "1", 1);
        TxDecisionRecord decisionRecord(2, key.getHash(), 6, 1,
    ...
    std::unique_ptr<Segment> recoverySegments(new Segment());
    TestLog::Enable _;
    build(buf, length, certificate, recoverySegments.get(), tableId,
    ...
    EXPECT_EQ("safeVersion at offset 0, length 12 with version 1 | "
              ...
              ObjectManager::dumpSegment(recoverySegments.get()));
...
TEST_F(MigrationSegmentBuilderTest, build_participantList)
{
    ...
    build(buf, length, certificate, migrationSegment.get(), tableId,
          firstKeyHash, lastKeyHash);

    EXPECT_EQ("safeVersion at offset 0, length 12 with version 1 | "
              "participantList at offset 14, length 96 with "
              "TxId: (leaseId 42, rpcId 9) containing 3 entries",
              ObjectManager::dumpSegment(migrationSegment.get()));
}
